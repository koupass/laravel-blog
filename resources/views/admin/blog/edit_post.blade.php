        ...
        <form action="{{route('admin.blog.post.update')}}" method="post">
            ...
            <div class="input-group">
                ...
                <input type="text" name="author" id="author"
                       {{$errors->has('title')?'class=has-error':''}} value="{{Request::old('author') ? Request::old('author') : isset($post)? $post->author : ''}}"/>
            </div>
            <div class="input-group">
            ...
            <div class="input-group">
                ...
                <textarea name="body" id="body" rows="12"
                        {{$errors->has('title')?'class=has-error':''}}>{{Request::old('body') ? Request::old('body') : isset($post) ? $post->body : ''}}</textarea>
            </div>
